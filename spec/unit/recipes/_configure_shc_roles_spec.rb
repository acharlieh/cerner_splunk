...
describe 'cerner_splunk::_configure_shc_roles' do
  subject do
    runner = ChefSpec::SoloRunner.new do |node|
      ...
      node.set['splunk']['config']['roles'] = 'cerner_splunk/roles'
    end
  ...
  it 'writes the authorize.conf file with the appropriate access and capabilities' do
    ...
    expect(subject).to create_splunk_template('shcluster/deployer-configs/authorize.conf').with(expected_attributes)
    expect(subject.splunk_template('shcluster/deployer-configs/authorize.conf')).to notify('execute[apply-shcluster-bundle]').to(:run)
  end
  ...
  it 'writes the user-prefs.conf file with the appropriate user preferences' do
    ...
    expect(subject).to create_splunk_template('shcluster/deployer-configs/user-prefs.conf').with(expected_attributes)
    expect(subject.splunk_template('shcluster/deployer-configs/user-prefs.conf')).to notify('execute[apply-shcluster-bundle]').to(:run)
  end
end
