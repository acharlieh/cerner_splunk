...
describe 'cerner_splunk::_configure_shc_authentication' do
  ...
  it 'writes the authentication.conf file with the appropriate strategy and role Map' do
    ...
    expect(subject).to create_splunk_template('shcluster/deployer-configs/authentication.conf').with(expected_attributes)
    expect(subject.splunk_template('shcluster/deployer-configs/authentication.conf')).to notify('execute[apply-shcluster-bundle]').to(:run)
  end
end
