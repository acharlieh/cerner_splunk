...
describe 'cerner_splunk::_configure_shc_outputs' do
  ...
  it 'writes the outputs.conf file with the appropriate configs' do
    ...
    expect(subject).to create_splunk_template('shcluster/deployer-configs/outputs.conf').with(expected_attributes)
    expect(subject.splunk_template('shcluster/deployer-configs/outputs.conf')).to notify('execute[apply-shcluster-bundle]').to(:run)
  end
end
